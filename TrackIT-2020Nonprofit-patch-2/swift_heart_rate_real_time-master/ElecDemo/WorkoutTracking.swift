                ...
                DispatchQueue.main.async {
                    ...
                    LocalNotificationHelper.fireHeartRate(heartRate)
                     let timer3 = Timer.scheduledTimer(withTimeInterval: 10.0, repeats: true) { timer in
                        ...
                        print("njshdjwhukehieuwhiuheiwjijeiwe")
                    }
                    let timer = Timer.scheduledTimer(withTimeInterval: 4.0, repeats: true) { timer in
                                                let defaults = UserDefaults.standard
                        
                                 
                                  var heartRateLvl =  defaults.integer(forKey: "heartRateLvl")
                        ...
                        self.healthStore.enableBackgroundDelivery(for: heartRateSampleType, frequency: .immediate) { (success, error) in
            print(success)
            if let error = error {
                print(error)
            }
...
extension WorkoutTracking : ColorServiceDelegate {
